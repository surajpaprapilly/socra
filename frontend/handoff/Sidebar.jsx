...
export default function Sidebar({
  ...
  const [expandedYears, setExpandedYears] = useState(() => {
    // Default: expand the two most recent years
    const years = PAST_YEAR_QUESTIONS.map(y => y.year);
    return { [years[0]]: true, [years[1]]: true };
  });
  ...
  return (
    <div className="w-[272px] flex-shrink-0 h-full flex flex-col border-r border-borderDark bg-[#11100D] overflow-hidden">
              ...
              questions.map((q, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left py-2 pr-3.5 border-l-2 transition-all duration-100 font-mono text-[10px] leading-snug line-clamp-2 ${
                      isSelected
      ...
      <div className="px-3.5 py-2.5 border-t border-borderDark flex items-center justify-between flex-shrink-0">
        <div className="flex gap-4">
          {[
            { val: blueprintCount, label: 'Blueprints' },
            { val: 4, label: 'This week' },
          ].map((s, i) => (
            <div key={i} className="text-center">
              <div className="font-display text-base text-textDefault font-bold leading-tight">
                {s.val}
              </div>
              <div className="font-mono text-[7px] uppercase tracking-wider text-textMuted/40">
                {s.label}
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="font-mono text-[8px] uppercase tracking-widest text-textMuted/55 border border-borderDark px-2.5 py-1.5 hover:border-amber hover:text-amber transition-all"
          >
            ◒ Profile
          </button>
          <button
            onClick={handleLogout}
            className="font-mono text-[8px] uppercase tracking-widest text-textMuted/25 hover:text-textMuted/55 transition-colors"
            title="Log out"
          >
            ⏻
          </button>
